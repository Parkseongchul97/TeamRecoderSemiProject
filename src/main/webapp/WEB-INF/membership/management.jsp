			...
			<table>
				...
				<tbody>
					...
					<c:forEach items="${allMember}" var="list">
						...
						<tr>
							<td id="${list.member.id}"
								class="${list.membership.membershipCode}">${list.member.id}</td>
							...
							<c:if
								test="${list.listGrade == 'admin' && not fn:contains(otherHost, list.member.id)  }">
								<!--  접속자가 호스트 인경우 -->
								<c:if test="${memberGrade == 'host'}">
									<form id="id${list.member.id}" class="fom">
										<td class="buttones"><input type="hidden" name="id"
											value="${list.member.id}"> <input type="hidden"
											name="membershipCode"
											value="${list.membership.membershipCode}">


											<button class="btn btn-dark btn-sm" name="listGrade"
												value="${list.member.id}" data-value="regular">일반회원</button>
											<button class="btn btn-danger btn-sm" name="listGrade"
												value="${list.member.id}" data-value="delete">추방</button>

											<button class="btn btn-warning btn-sm" name="listGrade"
												value="${list.member.id}" data-value="host">호스트</button></td>
									</form>
								</c:if>
								<!-- 접속자가 관리자인경우  -->
							...
							<c:if
								test="${list.listGrade == 'admin' &&  fn:contains(otherHost, list.member.id)  }">
								<!--  접속자가 호스트인 경우  -->
								...
								<c:if test="${memberGrade == 'host'}">
									<td class="buttones">
										<form id="id${list.member.id}" class="fom">
											...
											<input type="hidden" name="membershipCode"
												value="${list.membership.membershipCode}">
								...
								<form id="id${list.member.id}" class="fom">
									<td class="buttones"><input type="hidden" name="id"
										...
										<button class="btn btn-dark btn-sm" name="listGrade"
											value="${list.member.id}" data-value="regular">일반회원</button>
										<button class="btn btn-danger btn-sm" name="listGrade"
											value="${list.member.id}" data-value="delete">추방</button></td>
								</form>
							</c:if>
							...
							<c:if test="${list.listGrade == 'regular'}">
								<form id="id${list.member.id}" class="fom">
									<td class="buttones"><input type="hidden" name="id"
										value="${list.member.id}"> <input type="hidden"
										name="membershipCode"
										value="${list.membership.membershipCode}"> <c:if
											test="${memberGrade == 'host'}">
											<button class="btn btn-primary btn-sm" name="listGrade"
												value="${list.member.id}" data-value="admin">관리자</button>
										</c:if>
										<button class="btn btn-danger btn-sm" name="listGrade"
											value="${list.member.id}" data-value="delete">추방</button></td>
								</form>
